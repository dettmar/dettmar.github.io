...
let drawCharts = (workbook) => {
	...
	if(selector.selected_scope === "Region") {
		var lastYearData = data.filter(row => row["År"] === "2021")
		var latestWeek = lastYearData.map(row => row.Vecka).sort().pop()
		var lastWeekData = lastYearData.filter(row => row.Vecka === latestWeek) //  && row.Region !==  region
	...
	var genPies = genderStats.map(stat => {
		...
		return new Chart("gender_chart_relative_"+idGender, {
			...
			options: {
				...
				// 			console.log(tooltipItems, data, val)
				// 			//return data.datasets[tooltipItems.datasetIndex].label +': ' + tooltipItems.yLabel + ' personer';
				// 			return "bla" //data.datasets[tooltipItems.datasetIndex].label +': ' +
				// 					// (isPecentage ? tooltipItems.yLabel.toFixed(2) : tooltipItems.yLabel.toLocaleString()) +
				// 					// (isPecentage ? "%" : " personer");
				// 		}
				// 	}
	...
	return [progressionChart, horizontalBar, ageBar, genDonut, ...genPies]
}
...
var selector = new Vue({
	...
	data: {
		...
		selected_dose: 'Dos 1',
		dose_options: [{ text: 'dos nr. 1', value: 'Dos 1' }, { text: 'dos nr. 2', value: 'Dos 2' }]
	},
	methods: {
        onChange(event) {
